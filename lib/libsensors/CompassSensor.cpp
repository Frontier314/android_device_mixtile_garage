...
CompassSensor::CompassSensor()
    : SamsungSensorBase(NULL, "mmc3280",ABS_MISC)
 //     mEnabled(0),
 //     mInputReader(4),
 //     mHasPendingEvent(false)
{
//    AALOGE("CompassSensor::CompassSensor()");
...
//    AALOGE("CompassSensor::CompassSensor() open data_fd");
...
//    AALOGE("CompassSensor::~enable(0, %d)", en);
//    int flags = en ? 1 : 0;
//    if (flags != mEnabled) {
//        int fd;
//        strcpy(&input_sysfs_path[input_sysfs_path_len], "enable");
//        AALOGE("CompassSensor::~enable(0, %d) open %s",en,  input_sysfs_path);
//        fd = open(input_sysfs_path, O_RDWR);
//        if (fd >= 0) {
//             AALOGE("CompassSensor::~enable(0, %d) opened %s",en,  input_sysfs_path);
//            char buf[2];
//            int err;
//            buf[1] = 0;
//            if (flags) {
...
//    }
//
//    AALOGE("CompassSensor::~setDelay(%d, %lld) val = %d", handle, ns, val);
//
...
{
    //AALOGE("CompassSensor::~readEvents() %d", count);
    if (count < 1)
        return -EINVAL;
